...
class FCN(nn.Module):
    ...
    def forward(self, x):
        ...
        fs1 = self.refine6(F.upsample (gcfm1, fm3.size()[2:]) + gcfm2)
        fs2 = self.refine7(F.upsample (fs1, fm2.size()[2:]) + gcfm3)
        fs3 = self.refine8(F.upsample (fs2, pool_x.size()[2:]) + gcfm4)
        fs4 = self.refine9(F.upsample (fs3, conv_x.size()[2:]) + gcfm5)
        out = self.refine10(F.upsample (fs4, input.size()[2:]))
